...
func (bc *Blockchain) CreateTransaction(sender string, recipient string, value float32, senderPublicKey *ecdsa.PublicKey, s *utils.Signature) bool {
	...
	if isTransacted {
		for _, n := range bc.neighbors {
			publicKeyStr := fmt.Sprintf("%06x%06x", senderPublicKey.X.Bytes(), senderPublicKey.Y.Bytes())
			signatureStr := s.String()
			bt := &TransactionRequest{
				SenderBlockchainAddress:    &sender,
				RecipientBlockchainAddress: &recipient,
				SenderPublicAddress:        &publicKeyStr,
				Value:                      &value,
...
func (bc *Blockchain) ValidChain(chain []*Block) bool {
	...
	for currentIndex < len(chain) {
		...
		if !bc.ValidProof(b.Nonce(), b.PreviousHash(), b.Transactions(), MINING_DIFICULTY) {
			return false
		}
...
func (bc *Blockchain) ResolveConflicts() bool {
	...
	log.Printf("Resolver conflicts not replaced")
	return false
}
...
type TransactionRequest struct {
	...
	SenderPublicAddress        *string  `json:"sender_public_address"`
	Value                      *float32 `json:"value"`
	Signature                  *string  `json:"signature"`
}

func (tr *TransactionRequest) Validate() bool {
	if tr.SenderBlockchainAddress == nil || tr.RecipientBlockchainAddress == nil || tr.SenderPublicAddress == nil || tr.Value == nil || tr.Signature == nil {
		return false
	}
	return true
}
